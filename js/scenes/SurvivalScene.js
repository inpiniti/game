// ============================================================
//  RilyBox Game — Survival Scene (뱀서라이크 + "구원" 테마)
//  - 플레이어는 이동만, 주변 오라가 적을 지속 데미지로 '구원'(킬 대신 세이브)
//  - 적에게 스치면 즉사(보호막 있으면 무효화, 플레이어 HP 없음)
//  - 구원 수 누적 → 임계값 도달 시 3지선다 기독교 퀴즈 → 업그레이드 획득
//  - 조작: 화살표 / 왼쪽 조이스틱(이동만, 자동공격이라 액션 버튼 없음)
//  * 상수 U / F 는 MainScene.js 에서 전역 선언됨 (에셋 루트)
//  * QUIZ_DATA / UPGRADES / getRandomUpgrade 는 먼저 로드된 전역 참조
// ============================================================
...
class SurvivalScene extends Phaser.Scene {
    ...
    showQuiz() {
        ...
        // 미사용 인덱스 중 무작위 선택 (모두 소진 시 초기화)
        if (this.usedQuizIndices.size >= QUIZ_DATA.length) this.usedQuizIndices.clear();
        let idx;
        do { idx = Phaser.Math.Between(0, QUIZ_DATA.length - 1); } while (this.usedQuizIndices.has(idx));
        this.usedQuizIndices.add(idx);
        const quiz = QUIZ_DATA[idx];

        const w = this.scale.width, h = this.scale.height, D = 40000;
        const ui = [];
        ui.push(this.add.rectangle(w / 2, h / 2, w, h, 0x000000, 0.72).setScrollFactor(0).setDepth(D));
        ui.push(this.add.text(w / 2, h * 0.26, quiz.q, {
            fontSize: '26px', color: '#ffffff', align: 'center', stroke: '#000000', strokeThickness: 3,
            wordWrap: { width: Math.min(760, w - 80) }
        }).setOrigin(0.5).setScrollFactor(0).setDepth(D + 1));

        const btnW = Math.min(680, w - 60), btnH = 64, gap = 18, startY = h * 0.45;
        let answered = false;
        const buttons = [];
        quiz.choices.forEach((choice, i) => {
            const by = startY + i * (btnH + gap);
            const rect = this.add.rectangle(w / 2, by, btnW, btnH, 0x2b3a55, 0.95).setScrollFactor(0)
                .setDepth(D + 1).setStrokeStyle(2, 0xffffff, 0.4).setInteractive({ useHandCursor: true });
            const label = this.add.text(w / 2, by, choice, {
                fontSize: '22px', color: '#ffffff', align: 'center', wordWrap: { width: btnW - 30 }
            ...
            rect.on('pointerdown', () => {
                ...
                if (i === quiz.correct) {
                    ...
                    rect.setFillStyle(0x2e7d32, 1);
                    const up = getRandomUpgrade();
                    up.apply(this);
                    this.upgradeCounts[up.id] = (this.upgradeCounts[up.id] || 0) + 1;
                    this.quizToast(up.name);
                } else {
                    rect.setFillStyle(0x8e2b2b, 1);
                    buttons[quiz.correct].setFillStyle(0x2e7d32, 1);   // 정답만 초록 강조 (페널티 없음)
                }
                // time.timeScale=0 중에도 동작하도록 트윈 기반 지연 사용
                this.delayPaused(850, () => { ui.forEach((o) => o.destroy()); this.resumeGame(); });
            });
        });
    }
    ...
    showGameOver() {
        ...
        // 최고기록 비교/갱신
        const prev = JSON.parse(localStorage.getItem('rilybox_survival_best'));   // 없으면 null
        const isBest = !prev || sec > prev.time || (sec === prev.time && this.redeemedCount > prev.redeemed);
        if (isBest) localStorage.setItem('rilybox_survival_best',
            JSON.stringify({ time: sec, redeemed: this.redeemedCount }));
        const best = isBest ? { time: sec, redeemed: this.redeemedCount } : prev;

        const w = this.scale.width, h = this.scale.height, D = 40000;
        this.add.rectangle(w / 2, h / 2, w, h, 0x000000, 0.78).setScrollFactor(0).setDepth(D);
        this.add.text(w / 2, h * 0.24, '구원 종료', {
            fontSize: '40px', color: '#ffd700', stroke: '#000000', strokeThickness: 5
        }).setOrigin(0.5).setScrollFactor(0).setDepth(D + 1);
        this.add.text(w / 2, h * 0.47, [
            `생존 시간  ${this.fmtTime(sec)}`,
            `구원한 영혼  ${this.redeemedCount}`,
            `퀴즈 정답  ${this.quizStats.correct}/${this.quizStats.total}`,
            isBest ? '★ 최고 기록 갱신! ★' : `최고 기록  ${this.fmtTime(best.time)} · ${best.redeemed}`,
        ], {
            fontSize: '24px', color: '#ffffff', align: 'center', lineSpacing: 10, stroke: '#000000', strokeThickness: 3
        }).setOrigin(0.5).setScrollFactor(0).setDepth(D + 1);

        const by = h * 0.74;
        const btn = this.add.rectangle(w / 2, by, 260, 66, 0x2e7d32, 1).setScrollFactor(0)
            .setDepth(D + 1).setStrokeStyle(2, 0xffffff, 0.6).setInteractive({ useHandCursor: true });
        this.add.text(w / 2, by, '다시 시작', { fontSize: '26px', color: '#ffffff' })
            .setOrigin(0.5).setScrollFactor(0).setDepth(D + 2);
        btn.on('pointerdown', () => { this.resumeGame(); this.scene.restart(); });
    }
